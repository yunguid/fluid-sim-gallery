...
function main({ pane, contextID, glslVersion }) {
    ...
    const VORTEX_FORCE_BASE = 0.2;

    // Macro eddy and geometric mode constants
    const MACRO_EDDY_STRENGTH = 0.4; // Additional large-scale swirling on bass
    const MACRO_EDDY_RADIUS = 300;
    const MICRO_TURBULENCE_BURSTS = 5; // small bursts of micro-turbulence on strong beats
    const GEOMETRIC_MODE_DURATION = 120; // frames of geometric overlay after strong peaks

    // Damping and loudness handling
    const VELOCITY_DAMPING_FACTOR_NO_MUSIC = 0.99;
    const LOUDNESS_DURATION_THRESHOLD = 5 * 60; 
    const LOUD_VELOCITY_REDUCTION_FACTOR = 0.98;
    ...
    // Geometric mode toggle
    let geometricMode = false;
    let geometricModeTimer = 0;
    ...
    function visualizeAudio() {
        ...
        if (isLoud) {
            ...
            // Enter geometric mode on loud peaks
            geometricMode = true;
            geometricModeTimer = GEOMETRIC_MODE_DURATION;
        ...
        // Different frequency bands affect velocity and density
    ...
    // Macro-scale eddy force (large swirling pattern)
    function applyMacroEddy() {
        // Create a large swirl in the center or at edges for bass
        for (let i = 0; i < 4; i++) {
            const angle = i * (Math.PI / 2);
            const vector = [
                MACRO_EDDY_STRENGTH * currentMaxVelocity * Math.cos(angle),
                MACRO_EDDY_STRENGTH * currentMaxVelocity * Math.sin(angle)
            ];
            const position = [
                canvas.clientWidth * (0.5 + 0.4 * Math.cos(angle)),
                canvas.clientHeight * (0.5 + 0.4 * Math.sin(angle))
            ];
            ...
            composer.stepCircle({
                ...
                position,
                diameter: MACRO_EDDY_RADIUS,
            });
        }
    }

    // Micro turbulence bursts for high frequencies
    function applyMicroBursts(numBursts) {
        for (let i = 0; i < numBursts; i++) {
            const angle = Math.random() * 2 * Math.PI;
            const force = currentMaxVelocity * 0.3;
            const vector = [
    ...
    function applyAudioForces(frequencyRanges) {
        ...
        // Frequency-band forces:
        // Low freq (bass): big pushes, macro eddies if strong
        // Mid freq (melody): gentle rotational currents (already handled by vortex and stable flow)
        // High freq (cymbals): tiny sparks of turbulence and color flickers
        ...
        // Add random directional forces for variety
        const numForces = 3;
        ...
        if (low > 0.5) {
            // Strong bass triggers macro eddy
            applyVortexForces(frequencyRanges);
            applyMacroEddy();
        }

        if (high > 0.7) {
            // High frequencies cause micro turbulence bursts
            applyMicroBursts(MICRO_TURBULENCE_BURSTS);
        }
    }
    ...
    // Geometric overlay program
    const geometricOverlay = new GPUProgram(composer, {
        name: 'geometricOverlay',
        fragmentShader: `
        in vec2 v_uv;
        uniform float u_time;
        uniform sampler2D u_trailState;
        // We'll draw a subtle grid or lines pattern that quickly fades out
        out float out_color;

        void main() {
            float base = texture(u_trailState, v_uv).x;
            // Add a simple grid pattern
            float lineDensity = 50.0;
            float gx = step(0.01, fract(v_uv.x * lineDensity));
            float gy = step(0.01, fract(v_uv.y * lineDensity));
            float grid = (1.0 - gx) * (1.0 - gy);
            // Invert the logic: lines appear where fract <0.01
            float lines = (1.0 - grid)*0.5;

            // Blend lines onto existing trail
            out_color = clamp(base + lines * 0.3, 0.0, 1.0);
        }`,
        uniforms: [
    ...
    function loop() {
        ...
        // If geometric mode is active, overlay a pattern
        if (geometricMode) {
            geometricModeTimer--;
            geometricOverlay.setUniform('u_time', globalTime);
            composer.step({
                program: geometricOverlay,
                input: trailState,
                output: trailState,
            });
            if (geometricModeTimer <= 0) {
                geometricMode = false;
            }
        }
